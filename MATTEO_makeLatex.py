...
if __name__ == '__main__':
    ...
    if options.vbf:
       ...
       for lumi_float_value in Luminosities:
           ...
           LatexFileName = lumi_dir+"ExclusionLimits_lumi_%.0f_VBF.tex"%lumi_float_value;
           ...
           LatexFile.write("\item Luminosit\`a:%.0f ${fb}^{-1}$\n"%lumi_float_value);
           ...
           for sample in Samples:
               ...
               LatexFile.write("\\framesubtitle{Ntuple %s \\vspace{6pt} Channel: %s \\vspace{6pt} Luminosity: %.0f ${fb}^{-1}$}\n"%(Ntuple_Name_texttt,channel_latex_mm,lumi_float_value));
               
               k=0;
               for CutValue in CutInputVector:  
                     
                   
                   Plots_dir="DEta%1.1f_Mjj_%.0f/cards_%s_%s_VBF/%s/limitFigs"%(CutValue[0],CutValue[1],options.channel,options.category,sample);
                   ...
                   LatexFile.write("$\Delta\eta_{jj}=%1.1f$ \\hspace{10pt} $M_{jj}>%.0f$\n"%(CutValue[0],CutValue[1]));
