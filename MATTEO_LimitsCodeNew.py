...
def makeSMLimitPlot(SIGCH,cprime = 10, brnew = 00):
    ...
    for i in range(len(mass)):
        if options.mPDF:
           curFile = "higgsCombinewwlvj_BulkGraviton_newxsec%03d_%s%s_HP_unbin.HybridNew.mH%03d.root"%(mass[i],options.channel,SIGCH,mass[i]);
        else:        
...
#    hrl_SM.GetYaxis().SetNdivisions(505);
    ...
    #leg2.AddEntry(curGraph_xsec, "#sigma_{TH} #times BR_{G_{Bulk}#rightarrow WW}, k = 0.5","L") # MATTEO ADDED
